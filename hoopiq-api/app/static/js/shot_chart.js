...
function drawShotChart(shotData) {
    ...
    if (!shotData || shotData.length === 0) {
        ctx.fillStyle = 'rgba(90,122,99,0.5)';
        ctx.font = '10px IBM Plex Mono';
        ctx.textAlign = 'center';
        ctx.fillText('No shot data yet', cx, pad + cH / 2);
        drawLegend(ctx, W, H, pad, 0, 0);
        document.getElementById('shot-count-badge').textContent = '0 shots';
        return;
    }
...
function drawLegend(ctx, W, H, pad, makes, misses) {
    ...
    ctx.beginPath();
    ctx.arc(pad + 8, legendY, 5, 0, Math.PI * 2);
    ...
    ctx.fillText('swish', pad + 18, legendY + 4);

    ctx.beginPath();
    ctx.arc(pad + 75, legendY, 5, 0, Math.PI * 2);
    ctx.fillStyle = 'rgba(255,140,0,0.9)';
    ctx.fill();
    ctx.strokeStyle = '#ff8c00';
    ctx.stroke();
    ctx.fillStyle = 'rgba(255,140,0,1)';
    ctx.fillText('made' + (makes ? ' (' + makes + ')' : ''), pad + 85, legendY + 4);

    ctx.beginPath();
    ctx.arc(pad + 155, legendY, 5, 0, Math.PI * 2);
    ctx.fillStyle = 'rgba(255,68,102,0.7)';
    ctx.fill();
    ctx.strokeStyle = '#ff4466';
    ctx.stroke();
    ctx.fillStyle = 'rgba(255,68,102,0.9)';
    ctx.fillText('missed' + (misses ? ' (' + misses + ')' : ''), pad + 165, legendY + 4);
}
